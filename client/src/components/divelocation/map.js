...
class map extends React.Component {
  ...
  componentDidUpdate(prevProps){
    if (this.props.mapmarksGCS !== prevProps.mapmarksGCS) {
      ...
      const locatecurrentmark = marksinfo.filter(marksinfo=>marksinfo.LocationID == currentmark.LocationID)
      // console.log(locatecurrentmark)
      let mymap
      locatecurrentmark.forEach(function(locatecurrentmark){
          mymap = L.map("mapid").setView([locatecurrentmark.Latitude, locatecurrentmark.Longitude], 15);
      })
