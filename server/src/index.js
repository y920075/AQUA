...
app.use(async (req, res, next) => {
  ...
  const memberId =
    (await memberData.length) > 0 ? memberData[0].memberId : false;
  if (memberId) {
    req.session.memberId = memberId;
  }
  next();
});
const whitelist = [`http://localhost:${process.env.WEB_PORT}`, undefined];
console.log("CORS 白名單", whitelist);
const corsOptions = {
  credentials: true,
  origin: function (origin, callback) {
    if (whitelist.indexOf(origin) !== -1) {
      ...
      console.log("拒絕連線 from origin:", origin);
...
//引用顧客管理
app.use(
  "/seller/customermanager",
  require(__dirname + "/customermanagement/customermanagement")
);
...
app.listen(expressPort, () => {
  console.log(`express 伺服器啟動! ,port: ${expressPort}`);
});
