    ...
    if(key === 'basic_info') {
      ...
      return basic_info.map((value,index)=>{
          return(
            <div className="col-lg-6" key={index} >
            <div className="d-flex mb-3 mx-3 my-4 font-middle-chin">
                  <p className="mx-4"> <i className="fas fa-store mx-3"></i>賣場名:</p>
                  <p className="card-text mx-3">{value.seller_shop}</p>
                  </div>
                  <div className="mb-3 mx-3 my-4 font-middle-chin">
                  <p className="mx-4 my-4"> <i className="fas fa-store mx-3"></i>賣場描述:</p>
                  <p className="card-text mx-4">{value.seller_decrip}</p>
                  </div>
                  <div className="mb-3 mx-3 my-4 font-middle-chin">
                  <p className="mx-4 my-4"> <i className="fas fa-store mx-3"></i>賣場圖片:</p>
                  {( JSON.parse(value.shop_img)|| []).map((url,index) => {
                    return (<img key={index} width="100" height="100" src={(url.originalname) ? ("http://localhost:5000/images/shopimg/" + url.originalname) : ("https://images.unsplash.com/photo-1474203383385-8a34c1a55ebc?ixlib=rb-1.2.1&auto=format&fit=crop&w=967&q=80")} alt="..." />)
