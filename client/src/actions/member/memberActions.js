...
export const getUserCouponDetaiAsync = (usercoupondata) => {
  return async (dispatch) => {
    const request = new Request(
      `http://${process.env.REACT_APP_API_HOST}:${process.env.REACT_APP_API_PORT}/seller/customermanager/user-coupondata`,
      {
        method: 'GET',
        headers: new Headers({
          Accept: 'application/json',
...
export const memberLogoutAsync = () => {
  return async (dispatch) => {
    const request = new Request(
      `http://${process.env.REACT_APP_API_HOST}:${process.env.REACT_APP_API_PORT}/logout`,
      {
        method: 'POST',
      }
    )
